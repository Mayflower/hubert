...
module.exports = (robot) ->
    ...
    for github_url, github of robot.brain.data.octospy
      for repo_name, repo of github
        for event, listeners of repo
          if _.include(listeners, msg.message.user.room)
            watching.push
  ...
  robot.respond /octospy stop ([^ ]+\/[^ ]+) ?([^ ]*)? ?([^ ]*)?/i, (msg) ->
    ...
    for listener, i in listeners when listener == msg.message.user.room
  ...
  listen = (github_url, repo, event, msg) ->
    ...
    addListener = ->
      ...
      if (listener for listener in listeners when listener == msg.message.user.room).length == 0
        listeners.push msg.message.user.room
  ...
  robot.router.post '/hubot/octospy/:github/:event', (req, res) ->
    ...
    for room in (robot.brain.data.octospy[github_url]?[repo_name][event] || [])
      target =
        type: 'groupchat'
        room: room
